<?php
    ...
    if(isset($_GET)) {
        ...
        $sql = "SELECT * FROM modulo_ventilador WHERE id = $id";
        $qry = $pdo->query($sql);

        if ($qry->rowCount() > 0) {
            $ventilador_edit = $qry->fetchAll();
        }
    }
...
<body onload="return Animar()">
    ...
    <div class="container">
        ...
        <p><strong>Ops!</strong> Você não ativou o módulo de ventilador nas configurações. :(</p>
        </div>

        <div class="corpo-teste animated" id="texto">

            <div class="cabeçalho-modulo">
                <h1>Módulo Ventilador - Editar</h1>
            </div>

            <br>

            <form method="POST" action="models/modulo_ventilador.edit.php?id=<?php echo($ventilador_edit[0]['id']); ?>">
                <div class="form-group">
                    <label for="exampleInputDescricao">Descrição</label>
                    <input type="text" class="form-control" id="exampleInputDescricao" name="descricao" value="<?php echo($ventilador_edit[0]['descricao']); ?>">
