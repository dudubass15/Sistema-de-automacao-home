<?php
    ...
    if(isset($_GET)) {
        ...
        $sql = "SELECT * FROM modulo_computador WHERE id = $id";
        $qry = $pdo->query($sql);

        if ($qry->rowCount() > 0) {
            $computadores_edit = $qry->fetchAll();
        }
    }
...
<body onload="return Animar()">
    ...
    <div class="container-fluid">
        <div class="navbar-header">
            ...
            <a class="navbar-brand" href="modulo_computador.php">
    ...
    <div class="container">
        ...
        <div class="corpo-teste animated" id="texto">
            ...
            <div class="cabeçalho-modulo">
                <h1>Módulo Computadores - Editar</h1>
            </div>

            <br>

            <form method="POST" action="models/modulo_computador.edit.php?id=<?php echo($computadores_edit[0]['id']); ?>">
                <div class="form-group">
                    <label for="exampleInputDescricao">Descrição</label>
                    <input type="text" class="form-control" id="exampleInputDescricao" name="descricao" value="<?php echo($computadores_edit[0]['descricao']); ?>">
                    <small id="descricaoHelp" class="form-text text-muted">Identificação do sensor no ambiente onde foi instalado.</small>
                </div>

                <div class="form-group">
                    <label for="exampleInputDescricao">IP</label>
                    <input type="text" class="form-control" id="exampleInputip" name="ip" value="<?php echo($computadores_edit[0]['IP']); ?>">
                    <small id="IPHelp" class="form-text text-muted">Identificação do sensor no ambiente onde foi instalado.</small>
                </div>

                <div class="form-group">
                    <label for="exampleInputDescricao">Utilizador</label>
                    <input type="text" class="form-control" id="exampleInpututilizador" name="utilizador" value="<?php echo($computadores_edit[0]['utilizador']); ?>">
                    <small id="utilizadorHelp" class="form-text text-muted">Identificação do sensor no ambiente onde foi instalado.</small>
                </div>
